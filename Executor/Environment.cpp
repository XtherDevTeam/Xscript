    ...
    void Environment::LoadFromFile(const XString &FilePath, const XString &PackageName, bool IsMainPackage) {
        ...
        for (auto &Class: DependNativeClasses) {
            ...
            for (auto &Prefix: PathsToSearch) {
                XString FFF = Prefix + (Prefix.back() == std::filesystem::path::preferred_separator or Prefix == L"" ? L"" : (std::wstring){std::filesystem::path::preferred_separator}) + Class + DynamicLibraryPostfix;
                try {
        ...
        for (auto &Package: DependPackages) {
            ...
            for (auto &Prefix: PathsToSearch) {
                XString FFF = Prefix + (Prefix.back() == std::filesystem::path::preferred_separator or Prefix == L"" ? L"" : (std::wstring){std::filesystem::path::preferred_separator}) + Package;
